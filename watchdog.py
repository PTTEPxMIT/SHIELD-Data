...
class FileWatchdogHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        ...
        if event.event_type in ["created", "moved"]:
            # New file detected
            file_path = Path(
                event.dest_path if event.event_type == "moved" else event.src_path
            )
    ...
    def handle_new_file(self, file_path):
        ...
        commit_message = (
            f"Add {file_name} - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
        )
    ...
    def update_existing_branch(self, file_name, branch_name):
        ...
        commit_message = (
            f"Update {file_name} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def run_git(self, cmd, check=True):
        ...
        result = subprocess.run(
            full_cmd, cwd=REPO_ROOT, capture_output=True, text=True, check=False
        )
